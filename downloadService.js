...
function resolveOutputDir() {
  const cfgDir = config.get('outputDir');
  if (path.isAbsolute(cfgDir)) return cfgDir;
  return path.join(app.getPath('music'), cfgDir);
}

async function downloadAudio(event, url) {
  ...
  try {
    ...
    ensureDir(outDir);
    const audioFormat = config.get('audioFormat');
    const baseOut = path.join(outDir, `${title}.${audioFormat}`);

    let finalOutPath = baseOut;
    let counter = 1;
    while (fs.existsSync(finalOutPath)) {
      finalOutPath = path.join(outDir, `${title} (${counter++}).${audioFormat}`);
    }
